...
def create_session(db_url, db_type, user_id, label=None):
    ...
    try:
        ...
        if db_type == "postgres":
            cur.execute(
                f"INSERT INTO sessions (user_id, label) VALUES ({p},{p}) RETURNING id",
                (user_id, label))
            sid = cur.fetchone()[0]
        else:
            cur.execute(
                f"INSERT INTO sessions (user_id, label) VALUES ({p},{p})",
                (user_id, label))
            sid = cur.lastrowid
        conn.commit()
        return sid
...
def get_target_allocations(db_url, db_type, session_id):
    ...
    try:
        ...
        p = ph(db_type)
        cur.execute(
            f"SELECT * FROM target_allocation WHERE session_id = {p} ORDER BY id",
            (session_id,))
        rows = cur.fetchall()
        return dict_rows(cur, rows, db_type)
    finally:
...
def get_stocks(db_url, db_type, session_id):
    ...
    try:
        ...
        p = ph(db_type)
        cur.execute(
            f"SELECT * FROM stocks WHERE session_id = {p} ORDER BY id",
            (session_id,))
        rows = cur.fetchall()
        return dict_rows(cur, rows, db_type)
    finally:
...
def save_analysis_results(db_url, db_type, session_id, results):
    ...
    try:
        ...
        p = ph(db_type)
        cur.execute(
            f"DELETE FROM analysis_results WHERE session_id = {p}",
            (session_id,))
        for r in results:
            cur.execute(
